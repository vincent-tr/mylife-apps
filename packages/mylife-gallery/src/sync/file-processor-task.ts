...
import { DocumentCleanupTask } from './document-cleanup-task';
import { processFileWithLoader } from './loaders';
...
const VIDEO_EXTENSIONS = ['.webm', '.mpg', '.mp2', '.mpeg', '.mpe', '.mpv', '.mp4', '.m4p', '.m4v', '.avi', '.wmv', '.mov', '.qt', '.flv', '.swf'];

export class FileProcessorTask implements Task {
  constructor(private readonly list) {}
  ...
  private async processFile({ fullPath, relativePath, fileUpdateDate, size }) {
